...
def diagnose_ODRL(data_obj) -> tuple:
    ...
    graph = Graph()
    try:
        # Handle input whether it's passed as a JSON string or already a dict object
        if isinstance(data_obj, str):
            data_dict = json.loads(data_obj)
        else:
            data_dict = data_obj
            
        # Parse the JSON object directly into the RDF graph as JSON-LD
        graph.parse(data=json.dumps(data_dict), format="json-ld")
        format_name = "json-ld"
    except Exception as e:
        errors.append(f"FORMAT ERROR: The provided object could not be parsed as a valid JSON-LD / RDF graph. Details: {str(e)}")
        return errors, warnings, parsed_info, False

    if graph is None or len(graph) == 0:
        errors.append("FORMAT ERROR: The provided object resulted in an empty RDF graph.")
        return errors, warnings, parsed_info, False
